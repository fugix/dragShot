...
- **З кожним оновленням коду обов'язково підвищувати версію** у `package.json`.
- Формат версії: `1.0.x` — лише патч-число `x` збільшується на 1 від поточного значення.
- Поточна версія зберігається в `package.json` (`"version"`). Перед будь-яким кодовим оновленням прочитати її та записати `1.0.(x+1)`.
